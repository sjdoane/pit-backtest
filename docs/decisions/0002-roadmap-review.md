...
### Data-quality invariant 5 amendment footer (ADR 0017 snapshot universe, 2026-05-31)

Decision 12 invariant 5 ("every SP500 event ticker resolves to exactly one TICKERS row whose `[firstpricedate, lastpricedate]` interval contains the event date") is SUPERSEDED for the v1 universe by ADR 0017. The real Sharadar SP500 table publishes membership as quarterly `historical`/`current` snapshots (not only an `added`/`removed` event log), so the M5 universe rework reads membership from the snapshots and resolves each member ticker by DATE-AGNOSTIC ticker string. Invariant 5 is therefore reframed to "every SP500 snapshot member ticker resolves to exactly one TICKERS permaticker" (`Sp500SnapshotMembersResolveContract`); the date-interval-contains form is retired for snapshot members because a handful of legitimate quarter-end snapshots sit one to five trading days outside the member's price interval (spin-off when-issued and acquisition delisting-lag boundaries). The `added`/`removed` event log is validated separately by the new `Sp500AddedRemovedCrossCheckContract` (invariant 7). See ADR 0017 for the full reasoning and the empirical backing (all 1,155 distinct snapshot tickers resolve uniquely; the cross-check residual is zero).
