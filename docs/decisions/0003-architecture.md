...
### M4 PR 4 amendment footer (trial registry effective-N reconciliation, 2026-05-30)

Decision 21 of this ADR and the validation code sketch at line 609 carry two internal contradictions that M4 PR 4 (`validation/trial_registry.py`) reconciles:

1. **Default effective-N method**: the code sketch at line 609 reads "PCA-based by default; ONC is v1.1", while decision 21 reads "default is `naive_effective_n` ... PCA-based effective N for DSR is opt-in". Decision 21 is the locked decision and the reviewer's accepted critique (the "What the reviewer thinks the architecture gets wrong" section rejects PCA-as-default because PCA eigenvalues are noise at small trial counts). The naive method is the default; the line-609 sketch is superseded.

2. **PCA trial-count threshold**: decision 21 states both "opt-in for N>=50 trials" and "with N<30, PCA raises `InsufficientTrialsForPCAError`". The coherent reading: in the v1.1 PCA path, PCA is the recommended method at N>=50, hard-errors below N=30, and the 30-to-50 band is allowed-but-not-default.

M4 PR 4 ships the **naive method only**. PCA-based effective-N is structurally deferred to v1.1 because it operates on the per-trial return-series correlation matrix (Bailey-LdP 2014), and the v1 registry schema stores per-trial scalars only (`sr_hat`, `T`, `gamma_3`, `gamma_4`, `metadata`), not the return series. Any `effective_n_and_sr_variance(method="pca")` call raises `NotImplementedError` regardless of trial count; `InsufficientTrialsForPCAError` is exported as a reserved symbol for the v1.1 PCA path's genuine low-N case. `naive_effective_n` is supplied at `TrialRegistry.__init__` per decision 21 (the stub `__init__(self, db_path)` is amended to `__init__(self, db_path, naive_effective_n=1)`; this is a local change with no construction-site cascade, so it ships in the implementation PR without a separate prep PR). The scipy dependency that a real PCA would need stays unresolved per [ADR 0013](0013-psr-dsr-mintrl-public-api-and-bailey-ldp-2014-pin-correction.md) decision 11, to be re-litigated when v1.1 implements PCA.

### M4 PR 5 amendment footer (run_cpcv deferral to M5 + dataset_fingerprint policy, 2026-05-30)

**`Runner.run_cpcv` is deferred to M5.** The Validation code sketch at line 607-610 and the package layout name `run_cpcv` as an M4 deliverable, and the live stub at `src/pit_backtest/engine/runner.py:88-93` is `run_cpcv(self, cv_splitter: CPCVSplitter, bar_loop_factory: Callable[[], BarLoop]) -> BacktestPathDistribution[BacktestResult]`. The M4 PR 5 Plan + Plan-reviewer pass established that this signature is underspecified and the body cannot be faithfully implemented in M4:

1. `CPCVSplitter.split(observations, label_horizons)` requires both arguments to produce `Split`s, but `run_cpcv` takes neither, so the body cannot call `split()`.
2. The `bar_loop_factory: Callable[[], BarLoop]` is no-arg and cannot be parameterized per train/test split.
3. `BarLoop.run(start_dt, end_dt)` runs a contiguous calendar window with no fit/predict interface; a CPCV `Split` carries integer index sets (non-contiguous for `k_test >= 2`), which the date-range `run` cannot express.
4. A true CPCV path stitches out-of-sample test-fold predictions from a model trained on the complementary fold; the constant-weight BarLoop has no model to train. "What is a CPCV path for this engine" is answerable only once a fit/predict strategy exists, which arrives with the M5 single-factor momentum study (the actual CPCV consumer; fan chart).

The M4 CPCV acceptance criterion (ADR 0002: "CPCV with N=6, k=2 produces 5 paths and a `BacktestPathDistribution`") is satisfied by `CPCVSplitter.expected_path_count() == 5` + `path_assignments()` + the `BacktestPathDistribution` container (M4 PRs 2-3), NOT by the orchestration body. So `run_cpcv`'s deferral does not block M4 shipping. The stub raises a `NotImplementedError` naming the M5 dependency; the redesigned signature (which must take observations + label_horizons + a per-split parameterized factory) lands in M5 to avoid a blind signature churn now. A minimal "evaluate over each combination's test-window" body was explicitly rejected: it would honor purge/embargo at split time but not at evaluation time and would silently misrepresent CPCV to any fan chart that consumed it.

**dataset_fingerprint policy** (the derivation the M4 PR 4 registry deferred to this PR): the `analytics/result_adapter.py` adapter uses `demo.sharadar_bundle` (the bundle name, e.g. `sharadar_2026-05-29`) as the `TrialRegistry` partition key. The bundle name is already the project's dataset-version identity (`docs/methodology/dataset_versioning.md`). A content-hash fingerprint (parquet checksum) is a v1.1 refinement; v1 uses the human-readable bundle name so the registry key is stable and inspectable.

### Universe `membership_spells` semantics amendment footer (ADR 0017 snapshot universe, 2026-05-31)

Decision 6's `Universe` Protocol surface (`is_member`, `members_at`, `membership_spells`) is UNCHANGED by the M5 universe rework. The SEMANTICS of `membership_spells` change under the snapshot model (ADR 0017): the v1 `SharadarSP500Universe` reads membership from the Sharadar quarterly `historical`/`current` snapshots, so a spell is a maximal run of consecutive snapshot dates in which the asset is present, bounded by quarter-end snapshot dates (open-ended only for a current member). Spell boundaries are therefore accurate to the containing calendar quarter (effective-date error 0 to about 92 days) and a spell is NOT a tradable effective-date interval. `membership_spells` has no v1 consumer (the momentum signal and the survivorship example use `members_at`/`is_member`); it exists for audit and for the v1.1 effective-date reconciliation. See ADR 0017 decision 3.
