...
- **PR 4 pending**: `SharadarSP500Universe` real implementation with `is_member`, `members_at`, `membership_spells`; `SharadarDataSource.members_at(universe_id, dt)` wiring; `IsMemberAt(t)` demo with 2010 vs current S&P 500 count + survivor count + CAGR delta consistent with published studies.
- **PR 5 pending**: data quality contracts (5 invariants per `src/pit_backtest/data/contracts.py` Protocol surface); SF1-vs-SEP authoritative-source resolution; data freshness check at startup per ADR 0003 decision 16.
