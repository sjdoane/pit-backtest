...
- **PR D Phase 1 pending** (run perf-budget workflow on main via workflow_dispatch; collect 7-run median + stdev; commit empirical `.bench-baseline.json`; flip `bench/compare.py` to fail-on-regression at `max(20%, 3 * sigma)`).
