...
- **PR C2 pending** (active tolerance enforcement via Order.estimate_bps_at_submit + CostEstimateVsFillMismatchError + ADR 0011). Blocked on architectural decision (two cost-model instances vs shifted-dt lookup vs dormant-scaffold).
- **PR D pending** (bench/spy_20y.py on synthetic data + bench/compare.py + .github/workflows/perf-budget.yml + .bench-baseline.json + --impact-model=bouchaud CLI flag).
