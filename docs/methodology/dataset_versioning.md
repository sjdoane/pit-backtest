...
| Sharadar SFP | SFP | Fund Prices: same shape as SEP but for ETFs and CEFs. SEP returns empty for SPY/AGG/GLD queries; the pull script falls back to SFP. **Verified 2026-05-29 EOD: SFP's SPY coverage on Sharadar Premium starts 1997-12-31, NOT SPY's 1993-01-22 inception.** Pre-1998 SPY price data is unavailable from this vendor, so the M1 SPY reconciliation SI window (1993-01-22 to as_of) structurally SKIPS with current data; ADR 0008 lock #17's "SI in the kill-gate set" still holds (SKIPPED is not FAIL per ADR 0006 lock #6) but the empirical SI delta cannot be measured from Sharadar alone. v1.1 backlog item: source 1993-1997 SPY prices from CRSP via WRDS or from SSGA's historical NAV CSV if available. |
