...
The M2 PR C1 Runner enforces this contract: `_worker_run_one_param` at `src/pit_backtest/engine/runner.py` sets `os.environ["POLARS_MAX_THREADS"] = "1"` as the FIRST executable line of its body, before any Polars-importing module is loaded. Per ADR 0010 lock #5 a lint test at `tests/lint/test_runner_worker_polars_threads_first.py` AST-walks the function body and asserts the first executable statement is the env-var assignment. The lint also blocks `import polars` and `from polars import ...` at the runner module's top level so a future refactor cannot accidentally defeat the bootstrap. Spawn is forced on both Linux and Windows (no fork-on-Linux fallback) so the worker's interpreter is freshly initialized and the env-var assignment runs before Polars's thread pool is constructed.

**Single-worker fast-path carve-out (per post-impl reviewer Critical, M2 PR C1):** `Runner.run_sweep` includes a single-worker fast path (`num_workers == 1`) that runs `_worker_run_one_param` inline in the parent process to skip the spawn bootstrap cost in unit tests. The parent process's Polars thread pool may have ALREADY been constructed before the env-var assignment runs (because the runner module's own imports transitively load Polars). For the single-worker fast path the `POLARS_MAX_THREADS=1` invariant therefore does NOT hold in the parent; the worker's `pl.thread_pool_size()` reflects the parent's already-fixed pool size. The multi-worker spawn path (`num_workers >= 2`) IS the canonical contract because each worker is a fresh Python process whose interpreter has not yet imported Polars. Callers that need bit-identical determinism across `num_workers=1` runs must ensure the parent process's pool size is stable across runs (typically by constructing the Runner before any Polars DataFrame operation). The integration test at `tests/integration/test_spy_cost_sensitivity.py::test_run_sweep_multi_worker_spawn_enforces_polars_threads_1` exercises the spawn path with a probe factory that asserts `pl.thread_pool_size() == 1` inside each worker.
