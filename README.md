...
M1 shipped (kill gate passes on 1y/3y/5y/10y windows; SI structurally skipped per Sharadar Premium SPY data starting 1997-12-31). M2 PR A (cost-model math + Commission classes + golden fixture + ADR 0006/0007), M2 PR B (ImpactedPriceSource + SquareRootImpactMatchingEngine + BarLoop wiring + ADR 0009), and M2 PR C1 (SensitivityBand + Runner.run_sweep + spy_cost_sensitivity CLI demo + ADR 0010) shipped. M2 PRs C2 (active tolerance enforcement) and D (perf-budget CI on synthetic data) pending. Research phases 1 and 2 are complete; phase 3 ADRs 0001 (spec critique), 0002 (M1-M5 roadmap), 0003 (architecture), 0004 (rebalance calendar), 0005 (M2 plan), 0006 (trailing-period reconciliation), 0007 (FIM ceiling), 0008 (SSGA tolerances), 0009 (ImpactedPriceSource policy + M2 PR B structure), 0010 (sensitivity-band runner) are merged. See [`docs/ROADMAP.md`](docs/ROADMAP.md) and [`docs/decisions/`](docs/decisions/) for the current plan and locked architecture.
