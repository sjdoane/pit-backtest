...
M1 shipped (kill gate passes on 1y/3y/5y/10y windows; SI structurally skipped per Sharadar Premium SPY data starting 1997-12-31). M2 PR A (cost-model math + Commission classes + golden fixture + ADR 0006/0007), M2 PR B (ImpactedPriceSource + SquareRootImpactMatchingEngine + BarLoop wiring + ADR 0009), M2 PR C1 (SensitivityBand + Runner.run_sweep + spy_cost_sensitivity CLI demo + ADR 0010), M2 PR C2 (tolerance contract dormancy + ADR 0011 via 4-member council + verifier pass), and M2 PR D Phase 0 (perf-budget CI on synthetic data + `--impact-model=bouchaud` CLI flag + `BarLoop.timing_breakdown()` opt-in + ADR 0012 via 4-member council + verifier pass) shipped. M2 PR D Phase 1 (flip perf-budget workflow from warning to gate after empirical noise floor is committed) pending. Research phases 1 and 2 are complete; phase 3 ADRs 0001 (spec critique), 0002 (M1-M5 roadmap), 0003 (architecture), 0004 (rebalance calendar), 0005 (M2 plan), 0006 (trailing-period reconciliation), 0007 (FIM ceiling), 0008 (SSGA tolerances), 0009 (ImpactedPriceSource policy + M2 PR B structure), 0010 (sensitivity-band runner), 0011 (tolerance contract dormancy at M2), 0012 (perf-budget CI Phase 0 noise-floor measurement) are merged. See [`docs/ROADMAP.md`](docs/ROADMAP.md) and [`docs/decisions/`](docs/decisions/) for the current plan and locked architecture.
...
A 20-year backtest on 500 U.S. equity names completes in under 60 seconds on a laptop. The budget is tracked in CI via [`.github/workflows/perf-budget.yml`](.github/workflows/perf-budget.yml) using the `bench/spy_20y.py` synthetic harness (real Sharadar data is unavailable in CI per `dataset_versioning.md`). Per [ADR 0012](docs/decisions/0012-perf-budget-ci-phase-0-noise-floor.md) the workflow ships in two phases: **Phase 0** (current) runs the harness and emits a warning on regression; **Phase 1** (follow-up PR after empirical noise-floor data is collected on main) flips the workflow to fail on regression at `max(20%, 3 * sigma)` (revising the prior 10% threshold after the M2 PR D council pass, which empirically established that ubuntu-latest runners have 5-15% per-run variance and the legacy 10% threshold sat at the noise floor). Per ADR 0001 decision 18 the workflow is Linux-only; cross-platform reproducibility is disavowed.
